            ...
            <div className="space-y-3">
              {announcements.length === 0 ? (
                ...
                announcements.map((announcement) => (
                  <article key={announcement.id} className="min-w-0 rounded-2xl border bg-background p-4 shadow-sm">
                    ...
                    <p className="mt-2 break-words text-sm leading-6 text-muted-foreground [overflow-wrap:anywhere]">{announcement.content}</p>
                  </article>
                ))
              )}
            </div>
