  ...
  return (
    <div className="overflow-hidden rounded-2xl border bg-card shadow-sm">
      <div className="relative">
        <div className="relative aspect-[21/9] min-h-[260px] overflow-hidden bg-gradient-to-br from-amber-50 via-background to-stone-100 sm:min-h-[320px]">
          {active.image_path ? (
            // eslint-disable-next-line @next/next/no-img-element
            <img src={active.image_path} alt={active.title_ta} className="h-full w-full object-contain p-2 sm:p-3" />
          ) : (
        ...
        <div className="flex flex-col gap-3 border-t bg-card px-4 py-4 sm:px-5">
          ...
          <div className="min-w-0">
            ...
            {banners.length > 1 ? (
              <div className="flex items-center justify-between gap-2">
                <Button variant="ghost" size="sm" className="size-9 px-0" onClick={() => setIndex((index - 1 + banners.length) % banners.length)} aria-label="Previous banner">
                ...
                <Button variant="ghost" size="sm" className="size-9 px-0" onClick={() => setIndex((index + 1) % banners.length)} aria-label="Next banner">
